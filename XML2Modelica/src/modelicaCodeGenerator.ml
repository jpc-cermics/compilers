...
let parse_string token_fun lexbuf =
  try
    StringParser.parse_string token_fun lexbuf
  with
    | _ -> assert false
;;
...
let generate_attributes oc attribs =
  let rec modified_attributes attribs = match attribs with
    | [] -> []
    | (_attrib, value) :: attribs when value = "" ->
...
let generate_code init filename model =
  let generate_type_specifier oc t = match t.kind, t.output with
    ...
    | (FixedParameter | Parameter), _ when init ->
      Printf.fprintf oc " Real"
    | (FixedParameter | Parameter), _ when t.fixed <> "true" ->
      Printf.fprintf oc "  parameter Real"
    | (FixedParameter | Parameter), _ ->
      Printf.fprintf oc "  Real"
  ...
  let generate_start_attribute oc t = match t.kind with
    ...
    | Variable | DiscreteVariable when t.fixed <> "true" || not init ->
    ...
    | FixedParameter | Parameter ->
      ...
      else Printf.fprintf oc " = %s" t.initial_value
    | Input -> () in
