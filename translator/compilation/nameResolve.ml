...
and annotation =
  ...
  | UnknownAnnotation of (Location.t Syntax.class_modification) Lazy.t
...
and external_call = (external_call_desc, Location.t Syntax.externalll) node
...
and equation = (equation_desc, Location.t Syntax.equation option) node
...
and algorithm = (algorithm_desc, Location.t Syntax.algorithm option) node
...
and expression_information =
  {
    syntax: Location.t Syntax.expression option;
      ...
      location =
        {
          Location.start = 0;
                  Location.enddd = 0;
          Location.filename = Location.CommandLine
        }
    }
...
and resolve_toplevel_statements ctx node = match node.Syntax.nature with
  ...
  | Syntax.Within _path -> raise (CompilError
...
and resolve_type_specification ctx cl_spec =
  ...
  match cl_spec.Syntax.nature with
    ...
    | Syntax.EnumerationSpecifier (_idt, enum_comp, _) ->
...
and resolve_class_specification ctx encap part kind cl_spec =
  let ctx = {ctx with location = cl_spec.Syntax.info} in
  let resolve_specifier _encap' cl_def =
...
and resolve_extension_composition ctx _ctx' _id _cl_modif _comp =
  raise (CompilError
...
and resolve_external_call ctx extern =
  let resolve_external_call' extern' = match extern'.Syntax.nature with
    ...
    | Syntax.External (Some _lang, Some _extern_call, _, _) ->
        raise (CompilError
    ...
    let add_inverse_declaration arg acc =
      let add_inverse_declaration' expr modif =
        ...
        | Syntax.IndexedAccess (
            { Syntax.nature = Syntax.Identifier "inverse"; info = _; }, _),
          Syntax.Eq
            {
              Syntax.nature =
...
(*
and resolve_type_constraint _ctx elt_def = match elt_def.Syntax.nature with
...
and declared_elements ctx elt_def = match elt_def.Syntax.nature with
  | Syntax.ClassDefinitionElement (_, def, _) ->
      ...
      and loc = match def.Syntax.nature with
        | Syntax.Definition (_encap, _part, _kind, cl_spec) ->
...
and resolve_declaration ctx type_pref type_spec' dims decl cmt _type_spec =
...
and resolve_dimensions ctx subs =
  ...
  let resolve_dimensions' = function
    | None -> []
    | Some { Syntax.nature = Syntax.Subscripts subs_elts; info = _; } ->
...
and component_type ctx (flow, var, inout) base_type dims =
  ...
  let predefined_type_variability predef = match predef with
    | { Types.base_type = Types.RealType; attributes = _; } -> Types.Continuous
    | _ -> Types.Discrete in
  let rec class_specifier_variability cl_spec = match cl_spec with
    ...
    | Types.ClassType _cl_type -> Types.Continuous
    | Types.ComponentType cpnt_type -> evaluate cpnt_type.Types.variability
    | Types.ArrayType (_dim, cl_spec) -> class_specifier_variability cl_spec
    | Types.TupleType _cl_specs -> assert false in
...
and add_dimensions dims cl_spec =
  let add_dimension dim cl_spec = match dim with
    | Expression { nature = Integer i; info = _; } ->
...
and apply_element_redeclaration ctx _elt_desc _elt_type =
  raise (CompilError
...
and resolve_element_redeclaration ctx _cl_spec _each _final elt_def =
    ...
    let resolve_alternative' cpnt_type =
      ...
      match cl_spec with
        | Types.PredefinedType {
            Types.base_type = Types.BooleanType;
            attributes = _;
          } ->
...
and resolve_for_clause_e ctx equ for_inds equs =
  let range_element_type expr range =
    ...
    let sub_dimension cl_spec = match cl_spec with
      | Types.ArrayType (_dim, cl_spec) -> cl_spec
...
and resolve_when_clause_e ctx equ alts =
  let resolve_alternative (expr, equs) =
    ...
    let rec check_equation equ =
      ...
      let check_alternative (_expr, equs) =
        List.for_all check_equation equs in
      let check_function_call_e expr _fun_args =
      ...
      | Syntax.ForClauseE (_for_inds, equs) ->
          List.for_all check_equation equs
      | Syntax.ConnectClause (_expr, _expr') ->
          raise (CompilError
            {err_msg = ["_InvalidWhenEquation"];
             err_info = [];
             err_ctx = {ctx with location = equ.Syntax.info}}) (*error*)
      | Syntax.WhenClauseE _alts ->
          raise (CompilError
    ...
    | Types.ComponentElement _cpnt_type ->
        raise (CompilError
...
and equations ctx equ resolved_equation cpnt_type expr cpnt_type' expr' =
  ...
  let rec equations' i subs cl_spec expr cl_spec' expr' =
    match cl_spec, cl_spec' with
      ...
      | Types.ComponentType _cpnt_type, Types.ComponentType _cpnt_type' ->
          raise (CompilError
      ...
      | Types.TupleType _cl_specs, Types.TupleType _cl_specs' ->
          [{
            nature = Equal (expr, expr');
            info = Some equ
          }]
...
and resolve_function_argument_list ctx expr' named_elts args =
  let rec class_kind ctx =
    ...
    | SubscriptContext (ctx, _, _, _) | ForContext (ctx, _, _) ->
        class_kind ctx
    | _ -> None in
  let add_function_inout_argument ((_id, elt_type) as named_elt) inouts =
  ...
  let add_argument id arg arg' elt_type acc =
    let matchable_types cpnt_type cpnt_type' =
      ...
      let rec matchable_types' cl_spec cl_spec' = match cl_spec, cl_spec' with
        | Types.ArrayType (_dim, cl_spec), _ ->
            matchable_types' cl_spec cl_spec'
        | _, Types.ArrayType (_dim', cl_spec') ->
            matchable_types' cl_spec cl_spec'
        | _, _ ->
    ...
    | Types.ComponentElement cpnt_type'
        ...
        raise (CompilError
          {err_msg = ["_ArgVariabilityMismatch"];
           ...
           err_ctx = {ctx with location = arg.Syntax.info}}) (*error*)
    | Types.ComponentElement _cpnt_type' -> (id, arg') :: acc
    | _ -> raise (CompilError
        {err_msg = ["_ClassElemFoundInExpr"];
         err_info = [];
         err_ctx = {ctx with location = arg.Syntax.info}}) (*error*) in
  let named_arguments_of_arguments input_types args =
    let rec add_positional_arguments acc input_types args =
      ...
      | _, [] ->
          raise (CompilError
            {err_msg = ["_TooFewArgsInFuncCall"];
             ...
             err_ctx = ctx}) (*error*)
      | (id, elt_type) :: input_types,
        { Syntax.nature = Syntax.Argument arg; info = _; } :: args ->
      ...
      | _, { Syntax.nature = Syntax.NamedArgument _; info = _; } :: _ ->
      ...
      | _, [] ->
          raise (CompilError
            {err_msg = ["_TooFewArgsInFuncCall"];
             err_info = [];
             err_ctx = ctx}) (*error*)
      | _, { Syntax.nature = Syntax.Argument _; info = _; } :: _ ->
          raise (CompilError
            {err_msg = ["_MixedPositAndNamedFuncArgPass"];
             err_info = [];
             err_ctx = ctx}) (*error*)
      | _, { Syntax.nature = Syntax.NamedArgument (id, _); info = _; } :: _
          ...
          when not (List.mem_assoc id input_types) ->
            raise (CompilError
              {err_msg = ["_NonInputFuncArgElem"; id];
               err_info = [];
               err_ctx = ctx}) (*error*)
      | _, { Syntax.nature = Syntax.NamedArgument (id, arg); info = _; } :: args ->
  ...
  let resolve_built_in_function_argument arg = match arg with
    | { Syntax.nature = Syntax.Argument arg; info = _; } ->
  ...
  let built_in_function_inout_types ctx id (in_types, out_types) args' =
    ...
    let scalar_base_class_specifier (arg, arg') =
      let rec scalar_base_class_specifier' cl_spec = match cl_spec with
        | Types.ArrayType (_dim, cl_spec) -> scalar_base_class_specifier' cl_spec
        | _ -> cl_spec in
    ...
    let ndims arg' =
      ...
      let rec ndims' cl_spec =
        match cl_spec with
        | Types.ArrayType (_dim, cl_spec) -> ndims' cl_spec + 1
        | _ -> 0 in
       ...
       "reinit" | "delay"), _ when (class_kind ctx) = Some Types.Function ->
        raise (CompilError
          {err_msg = [id; "_OperCannotBeUsedWithinFuncDef"];
           err_info = [];
           err_ctx = ctx}) (*error*)
    | ("pre" | "edge" | "change"), [arg, _arg'] | "reinit", [(arg, _arg'); _]
...
and resolve_if ctx expr alts expr' =
  let resolve_data_expression ctx expr =
    let expr' = resolve_expression ctx expr in
    match expr'.info.type_description with
    | Types.ComponentElement _cpnt_type -> expr'
    | _ ->
        raise (CompilError
          {err_msg = ["_ClassElemFoundInExpr"];
           err_info = [];
...
and resolve_matrix_construction ctx _expr _exprss =
  raise (CompilError
          ...
          raise (CompilError
            {err_msg = ["_InvalidVarOfRangeExpr"];
             err_info = [("_Expr", Syntax.string_of_range start step stop);
                         ...
                         ("_VariabilityFound", var)];
             err_ctx = ctx})
      | Integer _i, Integer p, Integer _j when p = Int32.zero ->
          raise (CompilError
        ...
        Types.PredefinedTypeElement _ ->
          raise (CompilError
            {err_msg = ["_ClassElemFoundInExpr"];
             err_info = [];
             err_ctx = ctx}) (*error*)
  and class_specifier _expr expr' =
    match expr'.info.type_description with
...
and resolve_vector_reduction ctx expr expr' for_inds =
  let vector_reduction_type acc _expr expr' =
        ...
        Types.PredefinedTypeElement _ ->
          raise (CompilError
            {err_msg = ["_ClassElemFoundInExpr"];
             err_info = [];
             err_ctx = ctx}) (*error*)
  and range_element_type _range range' =
    let sub_dimension cl_spec = match cl_spec with
      | Types.ArrayType (_dim, cl_spec) -> cl_spec
      | Types.PredefinedType _ | Types.ClassType _ |
        Types.ComponentType _ | Types.TupleType _ ->
...
and resolve_and ctx expr arg arg' =
  let resolve_and' cpnt_type cpnt_type' =
    let and_type cl_spec cl_spec' = match cl_spec, cl_spec' with
      | Types.PredefinedType {
          Types.base_type = Types.BooleanType;
          attributes = _;
        },
...
and resolve_or ctx expr arg arg' =
  let resolve_or' cpnt_type cpnt_type' =
    let or_type cl_spec cl_spec' = match cl_spec, cl_spec' with
      | Types.PredefinedType {
          Types.base_type = Types.BooleanType;
          attributes = _;
        },
...
and resolve_comparison ctx expr kind arg arg' =
  let resolve_comparison' cpnt_type cpnt_type' =
    let comparison_type cl_spec cl_spec' = match cl_spec, cl_spec' with
      | Types.PredefinedType {
...
and resolve_power ctx expr arg arg' =
  let resolve_power' cpnt_type cpnt_type' =
    let power_type cl_spec cl_spec' = match cl_spec, cl_spec' with
      | Types.ArrayType
      ...
      | Types.ArrayType (dim,
          Types.ArrayType (dim',
            Types.PredefinedType {
              Types.base_type = Types.RealType;
              attributes = _;
            }
          )
        ),
...
and resolve_not ctx expr arg =
  let resolve_not' cpnt_type =
    let not_type cl_spec = match cl_spec with
      | Types.PredefinedType {
          Types.base_type = Types.BooleanType;
          attributes = _;
        } -> cl_spec
...
and element_nature_class _ctx = function
  | Types.ClassElement cl_spec -> evaluate cl_spec
  | Types.ComponentElement cpnt_type -> evaluate cpnt_type.Types.base_class
      ...
      | _ ->
          raise (CompilError
            {err_msg = ["_NotYetImplemented"; "_PredefinedTypeAttribModif"; id];
             err_info = [];
             err_ctx = ctx})
  and find_class_local_identifier flow _var _inout cl_type id =
...
(*
and resolve_lhs_expression ctx _expr =
  raise (CompilError
...
and element_nature_type _ctx elt_desc =
...
and expression_of_variable expr =
  ...
  | Syntax.IndexedAccess (expr', _subs) ->
...
and string_of_expression expr = match expr.nature with
  | BinaryOperation (_bin_oper_kind, expr, expr') ->
      Printf.sprintf "BinaryOperation(_, %s, %s)"
        (string_of_expression expr)
        (string_of_expression expr')
  | DynamicIdentifier (_i, _s) -> "DynamicIdentifier"
  | False -> "False"
  | FieldAccess (_expr, _s) -> "FieldAccess"
  | FunctionArgument _i -> "FunctionArgument"
  | FunctionCall (expr, exprs, expr') ->
      Printf.sprintf "FunctionCall(%s, {%s}, %s)"
        (string_of_expression expr)
        (String.concat "," (List.map string_of_expression exprs))
        (string_of_expression expr')
  | FunctionInvocation _exprs -> "FunctionInvocation"
  | If (_alts, _expr) -> "If"
  | IndexedAccess (_expr, _exprs) -> "IndexedAccess"
  | Integer i ->
      Printf.sprintf "Integer(%d)" (Int32.to_int i)
  | LocalIdentifier (i, s) ->
      Printf.sprintf "LocalIdentifier(%d, %s)" i s
  | LoopVariable _i -> "LoopVariable"
  | NoEvent _expr -> "NoEvent"
  ...
  | Real _f -> "Real"
  | String _s -> "String"
  | ToplevelIdentifier _s -> "ToplevelIdentifier"
  | True -> "True"
  | Tuple _exprs -> "Tuple"
  | UnaryOperation (_un_oper_kind, _expr) -> "UnaryOperation"
  | Vector _exprs -> "Vector"
  | VectorReduction (_exprs, _expr) -> "VectorReduction"
  | Coercion _ -> "Coercion"
