...
let string_of_classDefinitions _cl_defs = ""
...
let string_of_for_clause for_inds _algos =
  (string_of_for_inds for_inds) ^ " loop ... end for"

let string_of_while_clause expr _algos =
...
let string_of_conditional_equ alts _lgos =
  match alts with
  | [] -> ""
  | (expr, _lgos) :: _lts -> string_of_if_cond expr
...
let string_of_toplevel_element node = match node.nature with
  ...
  | VariablesDefinitions (expr, _subs, _cpnt_decls) ->
      string_of_expression expr
