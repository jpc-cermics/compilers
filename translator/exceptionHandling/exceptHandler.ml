...
let print_offset =
  function s ->
      Printf.printf "\n";
      for _i = 1 to 3 do print_string s done;
      Printf.printf " "
...
let display_header exn err_msg =
  let display_code exn err_msg =
    let rec display_code' err_msg =
      match err_msg with
        | [] -> ()
        | s :: _err_msg when s.[0] = '_' -> display (ErrorDico.getCode exn s)
...
let loc_info loc =
  match loc.Location.filename with
    | Location.CommandLine ->
        [("_Source", "_CommandLine");
         ("_CharacterPosition", string_of_int (loc.Location.start + 1))]
    | Location.LibraryFile lib_file ->
        let linenum, linebeg =
          Linenum.for_position lib_file loc.Location.start in
        [("_Source", lib_file);
         ("_LineNumber", string_of_int linenum);
         ("_ColumnNumber", string_of_int (loc.Location.start - linebeg + 1))]
  ...
  let handle' =
    match exn with
      ...
      | ErrorDico.GenericError { ErrorDico.err_msg = err_msg;
          ...
          display_info err_info;
          Printf.printf "\n"
      | Parse_error.SyntacticError { Parse_error.err_msg = err_msg;
                                Parse_error.err_info = err_info;
                                Parse_error.err_ctx = ctx } ->
          display_header exn err_msg;
          display_info (exec_step_info SyntacticAnalysis None);
          display_info
            (loc_info ctx.Parse_error.location);
          display_info err_info;
          Printf.printf "\n"
      | _ ->
