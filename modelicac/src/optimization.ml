...
let symbolic_expression_of_expression inl_par maps iexpr =
  ...
  and propagate_noEvent_into_if no_event expr expr' expr'' =
    let cond =
      if no_event then
        ...
        | BlackBox ("noEvent", [ScalarArgument _expr']) ->
            create_blackBox
              "noEvent"
...
let create_model' trace inl_par iexpr =
  ...
  in
  let get_parameter_info maps _s i = function
    ...
    | _ -> assert false
  and get_input_info _maps s i = function
    ...
    | _ -> assert false
  and get_variable_info maps _s i = function
    | Instantiation.InstantiatedVariable (
...
let print_model oc model =
  ...
  Array.iteri
    (fun i equation ->
      ...
      if equation.solved then SymbolicExpression.output oc (create_variable i)
      else Printf.fprintf oc "0";
      Printf.fprintf oc " = ";
      SymbolicExpression.output oc equation.expression;
      Printf.fprintf oc "\n")
    model.equations
