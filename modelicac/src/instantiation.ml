...
let string_of_expression  iexpr =
  ...
  and string_of_expression'' = function
    ...
    | ExternalFunctionCall (_name, _, _, _iexprs) -> "<funcall>"
...
and instantiate_main_components ctx modifs lccpnts =
  let instantiate_main_component modifs' ccpnt = match ccpnt with
...
and instantiate_components ctx modifs lccpnts =
  let instantiate_component modifs' ccpnt = match ccpnt with
      | Compilation.Parameter (param) ->
...
and instantiate_variable ctx modifs = function
    | Compilation.IntegerVariable attrs ->
      ...
      and make comment inout _flow ivalue vat_infos =
...
and merge_modifications modifs modifs' =
  ...
  and merge_optional_values cexpr_opt cexpr_opt' =
    match cexpr_opt, cexpr_opt' with
      | None, None -> None
      | Some _cexpr, (Some _ | None) -> cexpr_opt
      | None, Some _cexpr' -> cexpr_opt
...
and instantiate_expression ctx = function
  ...
  | Compilation.Reference (Compilation.ClassReference (_level, _strings)) ->
...
and check_function_type ctx lccl _iexprs =
  let extract_type = function
    | Compilation.IntegerVariable {
        Compilation.vat_dimensions = cdims;
        vat_nature = _;
        vat_inout = _;
        vat_comment = _;
        vat_modifications = _;
...
and instantiate_if_alternatives ctx tex_type alts =
  let instantiate_if_alternative (cexpr, cexpr') (tex_type, ialts) =
    let iexpr = instantiate_expression ctx cexpr
    and iexpr' = instantiate_expression ctx cexpr' in
...
and get_loop_variable_value ctx level = match ctx, level with
  | ForContext (_ctx', i), 0 -> Integer (Int32.of_int i)
  | ForContext (ctx', _i), _n -> get_loop_variable_value ctx' (level - 1)
...
and find_component_level ctx level = match ctx, level with
  ...
  | ClassContext (_ctx', licl), 0 -> Lazy.force licl
  | ClassContext (ctx', _), _n -> find_component_level ctx' (level - 1)
...
and search_into_parameter ctx s cs level path ipar =
  ...
  | (s, _cs) :: _, _ -> failwith ("find_parameter: parameter not found: " ^ s)
...
and search_into_compound_variable ctx s ics level path iexpr =
  ...
  match iexpr'.tex_type, iexpr'.tex_expression with
    ...
    | CompoundType dims, Some (Vector _iexprs) when Array.length dims > 0 ->
...
and find_subvector_dims ics = function
  | { tex_type = RealType dims; tex_expression = _; }
  | { tex_type = IntegerType dims; tex_expression = _; }
  | { tex_type = CompoundType dims; tex_expression = _; }
  | { tex_type = StringType dims; tex_expression = _; }
  | { tex_type = BooleanType dims; tex_expression = _; }
...
and get_compound_subvector ics iexpr =
  let rec get_compound_subvector' i iexpr =
    if i = Array.length ics then iexpr
      ...
      | { tex_type = _; tex_expression = Some (Vector iexprs); }
...
let rec list_of_array = function
  | { tex_expression = Some (Vector iexprs); tex_type = _; } ->
...
and flatten_components path icpnts =
  ...
  and flatten_parameter icpnts s = function
    | InstantiatedIntegerParameter (cmt, kind,
      ({ tex_type = IntegerType _dims; tex_expression = _; } as iexpr), pat_infos) ->
        let make iexpr =
    ...
    | InstantiatedStringParameter (cmt, kind,
      ({ tex_type = StringType _dimss; tex_expression = _; } as iexpr), pat_infos) ->
        let make iexpr =
          InstantiatedParameter
    ...
    | InstantiatedRealParameter
        (cmt,
         kind,
         ({ tex_type = (IntegerType _dims | RealType _dims);
            tex_expression = _; } as iexpr),
         pat_infos) ->
        let make iexpr =
          InstantiatedParameter
  ...
  and flatten_variable icpnts iinit_equs iequs s = function
    | InstantiatedDiscreteVariable (cmt, inout,
      ({ tex_type = (IntegerType _dims | RealType _dims);
         tex_expression = _; } as iexpr), infos) ->
        let make iexpr =
    ...
    | InstantiatedCompoundVariable (cmt,
      ({ tex_type = CompoundType _dims; tex_expression = _; } as iexpr)) ->
        let make iexpr =
...
and flatten_component_tree make path s iexpr =
  ...
  and flatten_subcomponents path iexprs i =
    if i > Array.length iexprs then []
      ...
      let path' = match iexprs.(i - 1) with
        | { tex_expression = Some (Vector _iexprs); tex_type = _; } ->
  ...
  in match iexpr with
    | { tex_expression = Some (Vector _); tex_type = _; } ->
...
and explode_compound_components ivars =
  let rec explode_compound_component icpnts iinit_equs iequs = function
    | path, InstantiatedVariable (InstantiatedCompoundVariable (_cmt,
      { tex_expression = Some (CompoundElement icl); tex_type = _; })) ->
        let icpnts', iinit_equs', iequs' =
...
and update_if_clauses path iif_clauses =
  let update_if_clause (iexpr, iequs) =
    let iexpr = update_typed_expression path iexpr
    and iequs = update_equations path iequs in
    iexpr, iequs
...
and update_when_clauses path iwhen_clauses =
  let update_when_clause (iexpr, iwhen_equs) =
    let iexpr' = update_typed_expression path iexpr
    and iwhen_equs' =
      List.map
        (function
...
and flatten_if_clauses iif_clauses =
  let flatten_if_clause (iexpr, iequs) =
    let iexpr = flatten_typed_expression iexpr
    and iequs = flatten_equations iequs in
    iexpr, iequs
...
and flatten_when_clauses iwhen_clauses =
  let flatten_when_clause (iexpr, iwhen_equs) =
    let iexpr' = flatten_typed_expression iexpr
    and iwhen_equs' =
      List.map
        (function
...
and flatten_multiplication iexpr iexpr' iexpr'' =
  let extract_subvector = function
    | { tex_expression = Some (Vector iexprs); tex_type = _; } -> iexprs
    | _ -> assert false
  ...
  | (IntegerType [| _n |] | RealType [| _n |]),
    (IntegerType [| _n'; _m'; |] | RealType [| _n'; _m'; |]) ->
      flatten_vector_by_matrix_product iexpr iexpr'
  | (IntegerType [| _n; _m; |] | RealType [| _n; _m; |]),
    (IntegerType [| _m' |] | RealType [| _m' |]) ->
      flatten_matrix_by_vector_product iexpr iexpr'
  | (IntegerType [| _n; _m; |] | RealType [| _n; _m; |]),
    (IntegerType [| _m'; _p'; |] | RealType [| _m'; _p'; |]) ->
      flatten_matrix_by_matrix_product iexpr iexpr'
  | _ -> assert false

and expand_identifier tex_type make _level iref =
...
and collect_connected_components =
  ...
  in function
    | [] -> []
    | FlowConnection (
        { tex_expression = Some iexpr; tex_type = _; },
        { tex_expression = Some iexpr'; tex_type = _; })
      :: iequs ->
...
and evaluate_cardinalities ss iequs =
  let rec evaluate_cardinalities_in_equation = function
    | Equation (iexpr, iexpr') ->
        ...
        Equation (iexpr, iexpr')
    | ConditionalEquation (_iif_clauses, _iequs) ->
        failwith
  ...
  and evaluate_cardinalities_in_expression' iexpr'' = function
    ...
    | Cardinality {
        tex_expression = Some (VariableValue (_, [(s, [||])]));
        tex_type = _;
      } ->
        {
          tex_type = IntegerType [||];
