...
let is_atomic expr = match nature expr with
...
let bufferize_rhs model_info tabs modes_on lhs expr =
...
*)
  ...
  and bufferize_rational_power expr expr' num =
    ...
    with
      | Not_found -> assert false
  and bufferize_partial_derivative _expr _expr' _expr'' = assert false
...
let bufferize_variable_store init _nb_pars nb_dvars nb_ders model_info =
  match snd init with
  | Some xml_filename ->
      begin
