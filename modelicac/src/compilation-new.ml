...
let rec compile_main_class pcls =
  let add_class (cls, funcs) (id, pcl) = match pcl.class_kind with
    ...
    | ParseTree.Class 
    | ParseTree.Model -> (id, pcl) :: cls, funcs
    | ParseTree.Record
    | ParseTree.Connector
    | ParseTree.Type
    | ParseTree.Package ->
...
and compile_equation ctx = function
  ...
  | ParseTree.ConnectClause (conn_ref, conn_ref') ->
      ...
      begin match var with
	    ...
	    | StringVariable _ 
	    | DiscreteVariable _ 
	    | RealVariable _ -> failwith "compile_equation: Invalid connection"
	    end
	| IntegerVariable _ 
	| StringVariable _ 
	| DiscreteVariable _ 
...
and find_connector ctx conn_ref = match ctx, conn_ref with
  | ClassContext (_, lccl), [(s, _array_subscr)]
    when List.mem_assoc s (Lazy.force (Lazy.force lccl).ccl_public_cpnts) ->
      ...
      with
        | Variable var -> var
        | Parameter _ -> failwith "find_connector: Bad reference"
      end
  | ClassContext _, [(s, _)] ->
      failwith ("find_connector: unknown identifier: " ^ s)
  | ClassContext (_, lccl),
    (s, _array_subscr) :: ([(_s', _array_subscr')] as conn_ref')
    when List.mem_assoc s (Lazy.force (Lazy.force lccl).ccl_public_cpnts) ->
      let ccl = Lazy.force lccl in
      begin match
        Lazy.force (List.assoc s (Lazy.force ccl.ccl_public_cpnts))
      with
        | Variable var -> 
	    (match var with
	    | CompoundVariable (lccl', _) ->
		let ctx' = ClassContext (ctx, lccl') in
		find_connector ctx' conn_ref'
	    | (IntegerVariable _|StringVariable _|DiscreteVariable _|RealVariable _) ->
		failwith "find_connector: Bad reference")
        | Parameter _ -> failwith "find_connector: Bad reference"
      end
  | ClassContext _, (s, _) :: _ ->
      failwith ("find_connector: unknown identifier: " ^ s)
  | ClassContext (_, _lccl), [] -> failwith "find_connector: empty reference"
  | ForContext (_, id), _ ->
      failwith
        ("find_connector: connector defined in a for loop on variable " ^ id)

  | ModificationContext _,_ 
  | TopLevelContext _,_  -> assert false

and compile_connection ctx conn_ref conn_ref' lccl _attrs _lccl' _attrs' =
  let create_connection (s, lcpnt) = match Lazy.force lcpnt with
    ...
    | Variable (IntegerVariable _|StringVariable _|DiscreteVariable _|
      CompoundVariable (_, _))
    | Parameter _ -> failwith "compile_connection: Invalid connector"
...
and compile_when_equation ctx = function
  | ParseTree.FunctionCallE ([("reinit", [||])], args_opt) -> 
      (match args_opt with
      | Some (ParseTree.ArgList ([expr1; expr] , None)) ->
	  (match expr1 with
	  | ParseTree.Reference cpnt_ref -> 
	      Reinit (
              compile_component_reference ctx cpnt_ref,
              compile_expression ctx expr)
	  | (ParseTree.Addition (_, _)|ParseTree.And (_, _)|ParseTree.Division (_, _)|ParseTree.End|ParseTree.Equals (_, _)|
	    ParseTree.ExpressionList _|ParseTree.False|ParseTree.FunctionCall (_, _)|ParseTree.GreaterEqualThan (_, _)|
	    ParseTree.GreaterThan (_, _)|ParseTree.If (_, _)|ParseTree.Integer _|ParseTree.LessEqualThan (_, _)|ParseTree.LessThan (_, _)|
	    ParseTree.ArrayConcatenation _|ParseTree.Minus _|ParseTree.Multiplication (_, _)|ParseTree.Not _|ParseTree.NotEquals (_, _)|
	    ParseTree.Or (_, _)|ParseTree.Plus _|ParseTree.Power (_, _)|ParseTree.Range (_, _, _)|ParseTree.Real _|ParseTree.String _|
	    ParseTree.Subtraction (_, _)|ParseTree.True|ParseTree.VectorOrRecord _) -> 
	      failwith "compile_when_equation: Invalid equation"
	  )

      | Some (ParseTree.ArgList ( _ ,_))
      | Some (ParseTree.NamedArgList ( _ ,_))
      | None -> failwith "compile_when_equation: Invalid equation"
      )

  | ParseTree.Equality (expr1, expr) -> 
      (match expr1 with
      | ParseTree.Reference cpnt_ref ->
	  Assign (
          compile_component_reference ctx cpnt_ref,
  ...
  | ParseTree.FunctionCallE ( _ , _) 
  | ParseTree.ConditionalEquationE _ 
  | ParseTree.ForClauseE _ 
  | ParseTree.ConnectClause _ 
  | ParseTree.WhenClauseE _  -> failwith "compile_when_equation: Invalid equation"
...
and compile_expression ctx expr =
  let rec compile_expression' = function
    ...
    | ParseTree.ExpressionList _exprs ->
    ...
    | ParseTree.LessThan (expr, expr') ->
        ...
        and cexpr' = compile_expression' expr' in
        GreaterThan (cexpr', cexpr)
    | ParseTree.ArrayConcatenation _exprss ->
    ...
    | ParseTree.Power (expr, expr') ->
        ...
        and cexpr' = compile_expression' expr' in
        Power (cexpr, cexpr')
    | ParseTree.Range (_expr, _expr', _expr_opt) ->
...
and to_argument cexpr =
  let rec array_dimensions expr = 
    let vect = is_vector expr in
    match vect with
    | Some cexprs -> Array.length cexprs :: array_dimensions cexprs.(0)
    | None -> [] in
  let rec flatten expr =
    ...
    | None -> [expr] in
  let vect = is_vector cexpr in
  match vect with
  | Some _ ->
      ArrayArgument (array_dimensions cexpr, Array.of_list (flatten cexpr))
  | None -> ScalarArgument cexpr

and is_vector exp = 
  match exp with
  | (Abs _|Acos _|Acosh _|Addition (_, _)|And (_, _)|Asin _|Asinh _|Atan _|
    Atanh _|Boolean _|Cardinality _|Cos _|Cosh _|Der _|Division (_, _)|
    Equals (_, _)|Exp _|ExternalFunctionCall (_, _, _)|Floor _|
    GreaterEqualThan (_, _)|GreaterThan (_, _)|If (_, _)|Integer _|Log _|
    Max (_, _)|Min (_, _)|Minus _|Mod (_, _)|Multiplication (_, _)|NoEvent _|
    Not _|NotEquals (_, _)|Or (_, _)|Power (_, _)|Pre _|Real _|Reference _|
    Sinh _|Sqrt _|String _|Subtraction (_, _)|Tan _|Tanh _|Time|Sin _) -> None
  | Vector cexprs -> Some cexprs 
...
and compile_component_reference ctx cpnt_ref =
  let rec compile_reference' ctx' level level' cpnt_ref = match ctx' with
    ...
    | ClassContext (_ctx'', _lccl) ->
        compile_component_path ctx ctx' level cpnt_ref
    | ModificationContext (ctx'', _lccl) ->
        begin try
...
and compile_subscripts ctx subscripts =
  let compile_subscript = function
...
and find_local_component_class_in lccl id =
  match find_local_component_in lccl id with
    | Variable var -> 
	(match var with
	| CompoundVariable (lccl, _) -> lccl 
	| (IntegerVariable _|StringVariable _|DiscreteVariable _|RealVariable _) ->
	    failwith "find_local_component_class_in: base type")
    | Parameter _ -> failwith "find_local_component_class_in: base type"

and is_base_type_parameter_of lccl id =
  match find_local_component_in lccl id with
    | Parameter (IntegerParameter _ | RealParameter _) -> true
    | Parameter (StringParameter _)-> false
    | Variable _ -> false

and is_base_type_variable_of lccl id =
  match find_local_component_in lccl id with
    | Variable (DiscreteVariable _ | RealVariable _) -> true
    | Variable (IntegerVariable _|StringVariable _|CompoundVariable (_, _)) -> false
    | Parameter _ -> false
...
and compile_component ctx i id pcpnt = match pcpnt.variability with
  ...
  | Some ParseTree.Constant -> 
      failwith "compile_component: only variables and parameters allowed"
...
and compile_annotation ann =
  ...
  and compile_initialization_arguments (weight, port_name, port_type) args =
    ...
    | ParseTree.ElementModification
        ...
        compile_initialization_arguments
          ...
          args'
    | ParseTree.ElementModification (_, _, _, _, _) :: args'
    | ParseTree.ElementRedeclaration (_, _, _)  :: args' -> 
        compile_initialization_arguments
          (weight, port_name, port_type)
          args' in
  let compile_port_annotation modif = match modif with
    ...
    | ParseTree.Modification (ParseTree.ClassModification _, Some _) 
    | ParseTree.Eq _
    | ParseTree.ColEq _ ->
	1.0, "", "" in
  ...
  let compile_imagine_annotation_argument (weight, port_name, port_type) arg =
    ...
    | ParseTree.ElementModification (_, _, _, _, _)
    | ParseTree.ElementRedeclaration (_, _, _) ->
	weight, port_name, port_type in
  ...
  | ParseTree.Annotation (ParseTree.ClassModification _ ) ->
      1.0, "", ""
